...
function aust_render_option_font(){
    $fonts = aust_fonts();
    $output = '';
...
function aust_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function aust_typography_enqueue_google_font($font) {
 
    if($font && $font != "---"){
        if(array_search($font, array_keys(aust_fonts())) > 17){
...
function aust_links_typography_font($json){
    if($json === null) return '';
    
    $links_fonts = '';
    $customize = (array)json_decode($json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= aust_typography_enqueue_google_font($customize['font_family_primary']);
    }
    if(isset($customize['font_family_second']) && $customize['font_family_second']){
        $links_fonts .= aust_typography_enqueue_google_font($customize['font_family_second']);
    }
    return $links_fonts;
}


function aust_options_patterns(){
    $output = '';
    $file_path = drupal_get_path('theme', 'aust');

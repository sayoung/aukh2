...
class element_gva_row{
			...
			'fields'	=> array(
		     ...
		     array(
		      ...
		      array(
		        'id'    => 'bg_row_curve',
		        'type'    => 'select',
		        'title'   => ('Add Curve for Row'),
		        'class'       => 'width-1-2',
		        'options'    => array(
		          	'' 					=> '-- None --',
		          	'row-curve-bottom-white' => 'Row Curve Bottom white',
		         )
		      )
		   )
		);
	...
	public function render_content( $settings = array(), $content = '' ) {
		...
		?>
		<?php ob_start() ?>
		<div class="gbb-row-wrapper section <?php print $row_class ?>" <?php if($row_id) print 'id="'.$row_id.'"' ?> style="<?php print $row_style ?>" <?php if($data_bg_video) print $data_bg_video; ?> >
			<?php if($row_text_overlay){ ?><div class="row-text-overlay"><?php print $row_text_overlay ?></div><?php } ?>
		  	<?php if($icon){ ?><span class="icon-row <?php print $icon ?>"></span><?php } ?>
		  	<div class="mourad <?php print $row_class ?> <?php print ($bg_particles=='on') ? ' row-background-particles-js' : ''; ?>"  >
		    	<div class="bb-inner <?php if($style_space) print $style_space; ?>">  
			  	...
			  	<?php if( $bg_attachment == 'fixed' ){ 
			  		...
			  		if($bg_position == 'center top'){
			  			$data_parallax = 'data-bottom-top="top: -20%;" data-top-bottom="top: 20%;"';
			  		}
			  	?>
